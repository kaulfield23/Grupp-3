...
function buildCardsForChallenges(challenges) {
  ...
  if (challenges.length === 0) {
    ...
    for (let i = 0; i < challenges.length; i++) {
      ...
      let Type = (newCard.querySelector(".challenges-type").innerHTML =
        challenge.type);
      let Title = (newCard.querySelector(".challenge-title").innerHTML =
        challenge.title);
      let Description = (newCard.querySelector(
        ".challenge-description"
      ).innerHTML = challenge.description);
      let MinParticipants = (newCard.querySelector(
        ".minParticipants"
      ).innerHTML = challenge.minParticipants + " - ");
      let MaxParticipants = (newCard.querySelector(
        ".maxParticipants"
      ).innerHTML = challenge.maxParticipants + " Participants");
      let Image = (newCard.querySelector(".challenge-picture").src =
        challenge.image);
      let Lables = (newCard.querySelector(".challenges-lable").innerHTML =
        challenge.labels);
